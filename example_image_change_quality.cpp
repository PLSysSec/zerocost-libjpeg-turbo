...
void put_scanline_someplace(JSAMPROW rowBuffer, int row_stride,
  std::add_lvalue_reference_t<JSAMPLE*> image_buffer,
  std::add_lvalue_reference_t<int> curr_image_row)
{
  int i;
...
int
write_JPEG (int quality,
  std::add_lvalue_reference_t<JSAMPLE*> image_buffer,
  std::add_lvalue_reference_t<unsigned char*> outfile,
  std::add_lvalue_reference_t<int> image_height,
  std::add_lvalue_reference_t<int> image_width,
  std::add_lvalue_reference_t<int> curr_image_row
)
{
   ...
   */
//   FILE* of;
//   if ((of = fopen("example_out.jpeg", "wb")) == NULL) {
...
//   fwrite(outbuffer, outsize, 1, of);
//   fclose(of);
//   outfile = (unsigned char*) malloc(outsize);
//   for(unsigned long i = 0; i < outsize; i++) {
//     outfile[i] = outbuffer[i];
//   }
...
)
{
  ...
  // if (fsize + 1 != sizeof(binaryData)) {
  //   printf("Data size doesn't match!\n");
  //   exit(2);
  // }

  // for(unsigned long i = 0; i < fsize + 1; i++) {
  //   if (fileBuff[i] != binaryData[i]) {
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    ...
    put_scanline_someplace(buffer[0], row_stride, image_buffer, curr_image_row);
  }
...
//   for(unsigned long i = 0; i < sizeof(outputData); i++) {
//     if (outfile[i] != outputData[i]) {
